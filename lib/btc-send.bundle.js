(() => {
  ...
  var CURVE_ORDER = /* @__PURE__ */ (() => Point.Fn.ORDER)();
  ...
  var hash160 = (msg) => ripemd160(sha2562(msg));
  ...
  function hexToBytes3(h) {
    ...
    return hex.decode(s.length % 2 ? "0" + s : s);
  }
  function bytesToHex3(b) {
    return hex.encode(b);
  }
  function p2wpkhScript(pubKeyCompressed) {
    const h = ripemd160(sha256(pubKeyCompressed));
    return OutScript.encode({ type: "wpkh", hash: h });
  }
  function compressedPubFromPriv(privHex) {
    const priv = hexToBytes3(privHex);
    const pub = secp256k1.getPublicKey(priv, true);
    return pub;
  }
  function buildSignedP2wpkhTx({ privKeyHex, fromAddress, toAddress, amountSats, feeSats, utxos }) {
    const priv = hexToBytes3(privKeyHex);
    const pub = secp256k1.getPublicKey(priv, true);
    const script = p2wpkhScript(pub);
    const tx = new Transaction();
    let total = 0;
    const selected = [];
    const need = Number(amountSats) + Number(feeSats);
    ...
    if (total < need) throw new Error("Insufficient BTC (need " + need + " sats incl fee, have " + total + ")");
    ...
    const change = total - Number(amountSats) - Number(feeSats);
    if (change >= 546) {
      tx.addOutputAddress(fromAddress, BigInt(change));
    } else if (change < 0) {
      throw new Error("Insufficient BTC after fee");
    } else if (change > 0) {
      // Refuse silent dust burn — ask the user to nudge the amount.
      throw new Error(
        "Change would be dust (" +
          change +
          " sats) — lower the BTC amount slightly so change is returned"
      );
    }
  ...
  globalThis.GladiatorBtc = { buildSignedP2wpkhTx, hexToBytes: hexToBytes3, bytesToHex: bytesToHex3, compressedPubFromPriv };
})();
